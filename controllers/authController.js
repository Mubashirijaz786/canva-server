...
exports.forgotPassword = async (req, res, next) => {
    try {
        ...
        try {
            ...
            console.error("❌ NODEMAILER ERROR (Forgot Pass):", emailErr.message);
    ...
    try {
        ...
        try {
            ...
            await sendEmail({
                ...
                html: `<h1>${otp}</h1><p>Super Admin is adding you as an admin. Share this code with them.</p>`
            });
            res.status(200).json({ message: "OTP sent to new admin's email!" });
        } catch (emailErr) {
            console.error("❌ NODEMAILER ERROR (OTP):", emailErr.message);
            return res.status(500).json({ 
